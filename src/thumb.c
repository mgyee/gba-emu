...
static int thumb_add_sub(Gba *gba, u16 instr) {
  ...
  u64 tmp;
  u32 res;
  bool carry = get_flag(&gba->cpu, CPSR_C);
  bool overflow = false;
...
static int thumb_add_cmp_mov_hi(Gba *gba, u16 instr) {
  ...
  if (opcode == 0x1) { // CMP
    ...
    set_flags(&gba->cpu, (u32)tmp, carry, overflow);
  } else {
    if (opcode == 0x0) { // ADD
      REG(rd) = op1 + op2;
    }
...
static int thumb_push_pop(Gba *gba, u16 instr) {
  ...
  if (l) {
    ...
    for (int i = 0; i < 8; i++) {
      if (list & (1 << i)) {
        REG(i) = bus_read32(gba, sp, access);
        access = ACCESS_SEQ;
        sp += 4;
      }
    }
...
static int thumb_ldm_stm(Gba *gba, u16 instr) {
  ...
  if (!list) {
    if (l) {
      PC = bus_read32(gba, REG(rb), ACCESS_NONSEQ);
      thumb_fetch(gba);
    } else {
      bus_write32(gba, REG(rb), PC, ACCESS_NONSEQ);
    }
    REG(rb) += 64;
  }
  ...
  if (l) {
    ...
    for (int i = 0; i < 8; i++) {
      if (list & (1 << i)) {
        REG(i) = bus_read32(gba, addr, access);
        access = ACCESS_SEQ;
        addr += 4;
      }
    }
...
int thumb_step(Gba *gba) {
  ...
  return thumb_lut[index](gba, instr);
}
